...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	public LoadAndDisplayImageTask(ImageLoaderEngine engine,
			ImageLoadingInfo imageLoadingInfo, Handler handler, String auth) {
		this.engine = engine;
	...
	private Bitmap decodeImage(String imageUri) throws IOException {
		...
		ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey,
				...
				options);
		return decoder.decode(decodingInfo, auth);
	}
	...
	private boolean downloadImage() throws IOException {
		InputStream is = getDownloader().getStream(uri,
				options.getExtraForDownloader(), auth);
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight)
		...
		if (targetFile != null && targetFile.exists()) {
			...
			Bitmap bmp = decoder.decode(decodingInfo, auth);
