...
public class BaseImageDecoder implements ImageDecoder {
	 ...
	 */
	@Override
	public Bitmap decode(ImageDecodingInfo decodingInfo, String auth)
			throws IOException {
		Bitmap decodedBitmap;
		ImageFileInfo imageInfo;

		InputStream imageStream = getImageStream(decodingInfo, auth);
		try {
			imageInfo = defineImageSizeAndRotation(imageStream, decodingInfo);
			imageStream = resetStream(imageStream, decodingInfo, auth);
	...
	protected InputStream getImageStream(ImageDecodingInfo decodingInfo,
		...
		return decodingInfo.getDownloader().getStream(
				...
				decodingInfo.getExtraForDownloader(), auth);
	}
	...
	protected InputStream resetStream(InputStream imageStream,
			ImageDecodingInfo decodingInfo, String auth) throws IOException {
		try {
			imageStream.reset();
		} catch (IOException e) {
			IoUtils.closeSilently(imageStream);
			imageStream = getImageStream(decodingInfo, auth);
		}
		return imageStream;
	}
